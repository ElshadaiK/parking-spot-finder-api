...
var { permissions, roles, users, slotStatus } = require('../config/migrations')
...
module.exports = {
    ...
    migrateStatus: async () => {
        logger.info(`Checking status migrations...`);

        await slotStatus.forEach(async single => {
            let statusDocumentCount = await statusModel.countDocuments({
                statusName: single.statusName
            })
            
                if(statusDocumentCount === 0) {
                        await statusModel.create({
                            ...single
                        })
                        logger.info(`completed ${single.statusName} status migrated...`);
                        
                }
        })
        logger.info(`completed status migrations...`);
    }
}